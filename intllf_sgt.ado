...
version 13
        ...
        * Intermediate values: cdf@y1, cdf@y2 
        ...
        qui gen double `zl' = 1 /*
        */ / [1+`q'*((exp(`sigma')*(1+`lambda'*sign(`xl')))/(abs(`xl')))^`p'] /*
        */                                              if inlist(`idx',3,4)
        
        qui gen double `zu' = 1 /*
        */ / [1+`q'*((exp(`sigma')*(1+`lambda'*sign(`xu')))/(abs(`xu')))^`p'] /*
        */                                              if inlist(`idx',2,4)
        
        qui gen double `Fl' = .5*(1-`lambda')                                 /*
        */ + .5*(1+`lambda'*sign(`xl'))*sign(`xl')*cond(`zl'<=`cond',         /*
        */ ibeta(1/`p',`q',`zl'), 1-ibeta(`q',1/`p',1-`zl'))                  /*
        */                                              if inlist(`idx',3,4)
        
        qui gen double `Fu' = .5*(1-`lambda')                                 /*
        */ + .5*(1+`lambda'*sign(`xu'))*sign(`xu')*cond(`zu'<=`cond',         /*
